  ...
  useEffect(() => {
    if (isPresent) {
      const enterAnimation = async () => {
        chevronAnimate(chevron.current, { x: [40, 0], opacity: 1 }, { duration: 0.3 });
        chevronAnimate('.Desc', { opacity: [0, 1] }, { duration: 0.6 });
      };
      enterAnimation();
    } else {
      const exitAnimation = async () => {
        await chevronAnimate(chevron.current, { x: [0, 40], opacity: 0 }, { duration: 0.3 });
...
const TitleSection = ({ setCategory, initialLength }: TitleSectionProps) => {
  ...
  const totalDatasLength = initialLength.film + initialLength.outsource + initialLength.short;
        ...
        <motion.div className="relative flex flex-wrap text-[calc(60px+4.5vw)] sm:text-[calc(20px+6.5vw)] font-bold leading-none">
          ...
          >
            {Array.from('Works').map((spell, idx) => (
              <motion.span key={idx} variants={titleChild} className="text-[#101010]">
                {spell}
              </motion.span>
            ))}
          </motion.span>
        </motion.div>
        <motion.div initial={'hidden'} animate={'visible'} variants={categoryContainer}>
          {categoriesInfo.map((info) => (
            <motion.div
              ...
              className={cls(
                categoryState === info.kind ? 'text-palettered' : 'text-[#bababa]',
...
const TagButton = ({ tag, css, onTagFunction }: TagButtonProps) => {
  ...
  useEffect(() => {
    if (isPresent) {
      ...
      const exitAnimation = async () => {
        await buttonAnimate(button.current, { y: [0, -20], opacity: [1, 0] }, { duration: 0 });
        safeToRemove();
      };
      exitAnimation();
    }
...
const TagButtonSection = ({ setSelectedTags, category }: TagButtonSectionProps) => {
  ...
  const insertHandler = (tag: string) => {
    setTags((p) => ({
      selected: [...p.selected, ...p.tagList.filter((el) => el.id === tag).map((el) => el.id)],
      tagList: p.tagList.map((el) => ({
        ...el,
...
const VideoTitlePresense = ({ title, description }: VideoTitlePresenseProps) => {
  ...
  useEffect(() => {
    if (isPresent) {
      const enterAnimation = async () => {
        ...
        animate('.Desc', { y: [40, 0], opacity: [0, 1] }, { duration: 0.4, ease: 'easeOut' });
        await animate(
          '.Title',
          { y: [-40, 0], opacity: [0, 1] },
      ...
      const exitAnimation = async () => {
        animate('.Desc', { x: [0, 40], opacity: [1, 0] }, { duration: 0.4, ease: 'easeIn' });
        await animate(
          '.Title',
          { x: [0, -40], opacity: [1, 0] },
...
const VideoDetail = ({
  ...
  useEffect(() => {
    if (category === 'film' || category === 'short') {
      fetch(`https://api.vimeo.com/videos/${resource.match(pattern)?.[1]}?fields=description`, {
        method: 'get',
        headers: {
      ...
      <div className="fixed overflow-y-scroll scrollbar-hide top-0 left-0 w-screen h-full p-4 bg-transparent">
        <div className="w-full py-16 bg-[#101010]">
          <div className="w-full flex xl:flex-nowrap flex-wrap justify-evenly gap-y-12">
            <div
              className={cls(
                category === 'short' ? 'aspect-[12/16] sm:aspect-video' : 'aspect-video',
              ...
              {category === 'film' || category === 'short' ? (
                <>
                  ...
                  {!isLoaded ? (
                    <div
                      className={cls(
                        category === 'short' ? 'h-full sm:aspect-video' : 'aspect-video',
            ...
            <div className="xl:min-w-[440px] h-auto w-full xl:max-w-[500px] xl:max-h-[calc(100vh-128px)] xl:w-[calc(100vw-1500px)] px-8 flex flex-col justify-between text-[#eaeaea]">
              <div className="font-GmarketSans font-semibold text-4xl cursor-default">
                ...
                <div className="text-xl text-[#dadada] right font-medium mt-6 sm:hover:text-palettered">
                  {role}
                </div>
                <div className="text-sm text-[#dadada] font-light mt-2 sm:hover:text-palettered">
                  {date}
                </div>
              </div>
                ...
                <Link
                  href={
                    category === 'outsource'
                      ...
                      : `https://vimeo.com/${vimeoId && vimeoId[1] ? vimeoId[1] : ''}`
                  }
                  target="_blank"
                >
                  {category === 'outsource' ? 'Youtube에서 보기' : 'Vimeo에서 보기'}
                </Link>
              </div>
            </div>
          </div>
...
const Video = ({
  ...
  useEffect(() => {
    if (titleScreen) {
      ...
      const exitAnimation = async () => {
        await coverAnimate(cover.current, { opacity: 1 }, { duration: 0.4, delay: 0.4 });
      };
      exitAnimation();
    }
  ...
  return (
    <>
      ...
      >
        {category === 'film' || category === 'short' ? (
          <div ref={videoRef} className="absolute w-full aspect-video bg-[#050505]">
        ...
        <AnimatePresence>
          {titleScreen ? <VideoTitlePresense title={title} description={description} /> : null}
        </AnimatePresence>
      </motion.article>
    </>
  );
...
const OutroSection = ({ isVisible }: OutroSectionProps) => {
  ...
  const onLinksEnter = (angle: number, selector: string) => {
    ...
    snsLinksAnimate(selector, { color: '#eaeaea', webkitTextStroke: '0px' }, { duration: 0.2 });
  };
...
export default function Work({ initialLength, initialWorks, initialHasNextPage }: initialData) {
  ...
  const [searchResults, setSearchResults] = useState<Videos<Works[]>>(initialWorks);
  const [hasNextPage, setHasNextPage] = useState<Videos<boolean>>(initialHasNextPage);
    ...
    if (tags.length === 1 && tags.some((tag) => tag === 'all')) {
      setSearchResults((p) => ({
        ...
        [category]: videos[category].filter((video) =>
          ciIncludes(video.title, useSnapshot ? searchWordsSnapShot : searchWords)
        ),
      }));
      if (
    ...
    const getVideos = async () => {
      ...
      if (tags.length === 1 && tags.some((tag) => tag === 'all')) {
        setSearchResults((p) => ({
          ...
          [category]: [
            ...p[category],
            ...lists.works[category].filter((li) => ciIncludes(li.title, searchWordsSnapShot)),
          ],
        }));
      } else {
        setSearchResults((p) => ({
  ...
  const intersectionRef = useInfiniteScroll({
    ...
    dependencyArray: [category, page, hasNextPage[category], isAnimationEnd, fetchLoading],
  });
    ...
    <>
      ...
      >
        ...
        >
                  ...
                  idx < perPage * (page - 1) ? (
                    <div key={data.id}>
                      ...
                      <div className="block sm:hidden">
                        ...
                        <div className="font-light text-xs">{data.description}</div>
                      </div>
                    </div>
                  ) : null
                )}
          ...
          <OutroSection isVisible={page > searchResults[category].length / perPage + 1} />
