...
export default function Contact() {
  ...
  useEffect(() => {
    const enterAnimation = async () => {
      await animate('.Title', { x: [-50, 0], opacity: [0, 1] }, { duration: 0.3 });
      animate('.Input', { x: [100, 0], opacity: [0, 1] }, { delay: stagger(0.2) });
      await animate('.Letters', { x: [-50, 0], opacity: [0, 1] }, { duration: 0.3 });
      await animate('.Links li', { x: [-50, 0], opacity: [0, 1] }, { delay: stagger(0.1) });
      animate(
        '.Title > div > span',
  ...
  const [send, { loading, data }] = useMutation<{ success: boolean }>('/api/email');
            ...
            <div className="Links font-light text-[calc(10px+0.75vw)] leading-none ">
              <ul className="lg:block hidden space-y-6">
                ...
                <li className="opacity-0 hover:text-palettered transition-colors">
                  <Link href="https://www.instagram.com/yarg__gray" target="_blank">
                    INSTAGRAM
                  </Link>
                </li>
                <li className="opacity-0 hover:text-palettered transition-colors">
                ...
                <li className="opacity-0 hover:text-palettered transition-colors">
                  <Link href="https://www.youtube.com/@insan8871" target="_blank">
                    YOUTUBE
                  </Link>
                </li>
              </ul>
        ...
        <div className="relative w-full flex items-center justify-center pb-16 lg:pb-0">
          ...
          >
            ...
            <div className="space-x-4">
              <span
                className={cls(
                  resultMessage.state === 'resolve' ? 'text-green-500' : 'text-palettered',
