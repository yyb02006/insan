...
const SpringTextMotionLi: NextPage<springTextMotionLiProps> = ({
  ...
  const x = useTransform(mouseX, (offset) => (innerWidth > 640 ? offset / 15 : null));
  const y = useTransform(mouseY, (offset) => (innerWidth > 640 ? offset / ratio : null));
  return (
    <m.li
      style={{
        x,
...
const SpringText = ({ mouseX, mouseY, scrollYProgress, innerWidth }: SpringTextProps) => {
    ...
    >
      ...
      >
        {[
          ...
          <motion.li key={idx} variants={snsList} className="hover:text-palettered">
...
const CircleSection: NextPage<HeaderProps> = ({
  ...
  const rotate = useTransform(scrollYProgress, [0, 1], [0, isMobile ? 270 : 360], {
    clamp: false,
  });
  const y = useTransform(scrollYProgress, [0.1, 0.4], [0, -200]);
  const scale = useTransform(scrollYProgress, [0.1, 0.4, 0.7, 0.9], [1, 0.5, 0.5, 0]);
  const logoCircle = useTransform(scrollYProgress, [0.7, 0.9], [1, 0]);
  const circleLineScale = useTransform(mouseY, (offset) => 1 + Math.abs(offset / 20000));
  const logoCircles = useRef([
    'top-[-220px] left-[-200px] w-[360px] lg:w-[460px]',
    'top-[-120px] left-[-80px] w-[240px] lg:w-[340px]',
  ]);
  return (
    <section className="relative h-[900vh] sm:h-[500vh] mb-[100vh]" ref={inheritRef}>
          ...
          <m.div style={{ scale: logoCircle }} className="sticky top-0">
            {logoCircles.current.map((arr, idx) => (
              <m.div
                ...
                animate="visible"
                variants={sideCircle}
                className={cls(arr, 'absolute border rounded-full border-[#bababa] aspect-square')}
              />
            ))}
          </m.div>
        ) : null}
...
const Wave: NextPage<WaveProps> = ({
  ...
  const y = useTransform(scrollYProgress, [startHeight, endHeigth], [20, letterHeightFix]);
    ...
    >
      ...
      <div className="absolute w-full flex justify-center h-auto">
        <m.div
          ...
          className={cls(
            css,
            'select-none relative w-full max-w-[1920px] top-9 sm:top-7 md:top-5 lg:top-3 xl:top-0 flex px-[10vw] font-Roboto font-black text-[calc(14px+5.5vw)] text-[#fafafa]'
          )}
        >
          {waveReverse
      ...
      <div className="w-[100vw] h-[10vw] overflow-hidden">
        <m.div
          ...
          className={cls(
            index === 1 ? 'bg-wave-pattern-gradation1-4 sm:bg-wave-pattern-gradation1-2' : '',
            index === 2 ? 'bg-wave-pattern-gradation2-4 sm:bg-wave-pattern-gradation2-2' : '',
            index === 3 ? 'bg-wave-pattern-gradation3-4 sm:bg-wave-pattern-gradation3-2' : '',
...
const WavesSection: NextPage<WaveSectionProps> = ({ scrollYProgress, inheritRef, innerWidth }) => {
...
const VideoContainer: NextPage<VideoContainerProps> = ({
  ...
  useEffect(() => {
      ...
      motionInfos.forEach((motionInfo) => {
        textAnimate(
          motionInfo.kind,
          {
            ...
            y: reverse
              ? [0, motionInfo.move === 'up' ? motionInfo.range : -motionInfo.range]
              : [motionInfo.move === 'up' ? motionInfo.range : -motionInfo.range, 0],
          },
    ...
    if (isInView) {
      videoAnimate('.Wrapper', { opacity: [0, 1] }, { duration: 0.4, ease: 'easeOut' });
      if (innerWidth > 640) {
        textMotion();
      }
    } else {
      videoAnimate('.Wrapper', { opacity: [1, 0] }, { duration: 0.4, ease: 'easeIn' });
      if (innerWidth > 640) {
        textMotion(true);
      }
    }
  ...
  return (
    <section ref={ref} className="h-[100vh] w-screen">
      <div className="relative h-[100vh] flex justify-start items-center">
        ...
        <div ref={videoScope} className="w-[100vw-48px] m-auto sm:w-[600px] sm:ml-[calc(20vw)]">
        ...
        >
          <div className="flex justify-end items-center">
            ...
            <div className="Title font-GmarketSans font-bold absolute text-[6.5vw]">{title}</div>
          </div>
...
const VideosSection: NextPage<VideoSectionProps> = ({ innerWidth, isMobile }) => {
  const dummyDatas = [
    {
      index: 1,
      title: 'EMOTIONAL',
  ...
  return (
    <section ref={vertical} className="relative h-[600vh] sm:h-[600vh]">
      <div ref={inner} className="top-0 absolute h-[calc(100%-100vh)] w-full"></div>
      <div className="sticky top-0 overflow-hidden">
        <m.div ref={horizental} style={{ x }} className="text-[200px] h-[100vh] w-[400vw] flex">
          {dummyDatas.map((data) => (
            <VideoContainer
              key={data.index}
              index={data.index}
      ...
      <AnimatePresence>
        {isInView ? (
          <VideoSectionIndicator scrollYProgress={scrollYProgress} innerWidth={innerWidth} />
        ) : null}
      </AnimatePresence>
    </section>
  );
...
const VideoSectionIndicator: NextPage<VideoSectionIndicatorProps> = ({
  ...
  useEffect(() => {
    if (isPresent) {
      ...
      const exitAnimation = async () => {
        await animate(indicator.current, { x: 0, opacity: 0 }, { duration: 0.2 });
        safeToRemove();
      };
      exitAnimation();
    }
  ...
  return (
    <div ref={indicator} className="fixed left-0 top-0 h-full w-[28px] hidden sm:flex items-center">
...
const TextSectionMotionSpan: NextPage<TextSectionMotionProps> = ({
  ...
  const opacity = useTransform(scrollYProgress, [scrollStart, scrollEnd], [0, 1]);
  return (
    <m.span
      style={{
        y,
      ...
      <div className="relative font-GmarketSans font-bold leading-[1.2] sm:leading-[1.4] lg:leading-[1.3] xl:leading-[1.2] text-[#101010] text-[calc(16px+9vw)] pr-0 sm:pl-10 md:pr-20 ">
        ...
        <div className="flex flex-col text-[calc(16px+4vw)] text-[#dadada] -mt-0 space-y-2 sm:space-y-0 sm:-mt-6 mb-2 sm:-mb-2 -ml-6 sm:-ml-16">
          <TextSectionMotionSpan
            ...
            scrollEnd={0.25}
          >
            좋은 영상을 <div className="font-extralight sm:inline">만든다는 것은,</div>
          </TextSectionMotionSpan>
          <TextSectionMotionSpan
            scrollYProgress={scrollYProgress}
            scrollStart={0.2}
            scrollEnd={0.3}
          >
            당신께 감동을 <div className="font-extralight sm:inline">드린다는 것.</div>
          </TextSectionMotionSpan>
        </div>
        <TextSectionMotionSpan
          scrollYProgress={scrollYProgress}
        ...
        <div className="relative flex flex-col text-[calc(16px+4vw)] text-[#dadada] -mt-0 space-y-2 sm:space-y-0 sm:-mt-6 mb-2 sm:-mb-2 -ml-6 sm:-ml-16">
          ...
          <TextSectionMotionSpan
            ...
            css="relative"
          >
            상상이 현실이 <div className="font-extralight sm:inline">되는 감동을,</div>
          </TextSectionMotionSpan>
          <TextSectionMotionSpan
            scrollYProgress={scrollYProgress}
            scrollStart={0.37}
            ...
            css="relative"
          >
            더 나은 컨텐츠로의 <div className="font-extralight sm:inline">영감을.</div>
          </TextSectionMotionSpan>
        </div>
        <TextSectionMotionSpan
          scrollYProgress={scrollYProgress}
...
/* react.memo 사용 예시 */
const OutroSection: NextPage = React.memo(() => {
  OutroSection.displayName = 'OutroSection';
    ...
    >
      ...
      >
        ...
        <CircleSection
          ...
          isMobile={isMobile}
        />
        <WavesSection inheritRef={wave} scrollYProgress={scrollYProgress} innerWidth={innerWidth} />
