...
import { useAnimate, motion, useScroll, useTransform, useInView, MotionValue } from 'framer-motion';
...
const ScrollTransformDiv = ({
  ...
  const y = useTransform(scrollYProgress, [0, scrollEnd], [valueStart, valueEnd]);
...
const HeaderSection = () => {
  ...
  useEffect(() => {
    const enterAnimation = async () => {
      await headerAnimate('.Line', { width: ['0%', '100%'] }, { duration: 0.6, ease: 'easeInOut' });
      headerAnimate('.TopLetter', { y: [100, 0], opacity: [0, 1] }, { duration: 0.4 });
      await headerAnimate('.BottomLetter', { y: [-100, 0], opacity: [0, 1] }, { duration: 0.4 });
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Circles liMotion={{ css: 'w-[calc(28px+100%)] sm:w-[calc(50px+100%)]' }} />
        </ScrollTransformDiv>
      </motion.div>
      <ScrollTransformDiv
        scrollYProgress={scrollYProgress}
          ...
          >
            {Array.from('저는 이런 일을 합니다').map((letter, idx) => {
              if (idx > 2 && idx < 7) {
                ...
                return (
                  <motion.span variants={waveChild} className="text-[#101010]" key={idx}>
                    {letter === ' ' ? '\u00A0' : letter}
                  </motion.span>
                );
              }
      ...
      >
        {descDatas.map((data, idx) => (
          <WorkIntroSectionMotionLi
            ...
            css={cls(idx % 2 === 0 ? 'justify-end' : 'justify-start', 'flex w-full')}
          >
            {data.firstLetter}
            <span className="font-thin text-[#bababa]">{data.secondLetter}</span>
          </WorkIntroSectionMotionLi>
        ))}
      </ul>
    </section>
...
const RoleIntroSection = () => {
  ...
  useEffect(() => {
    if (isInview) {
      animate('.Image', { y: 0, scale: 1, opacity: 1 }, { duration: 0.6, ease: 'easeInOut' });
    }
  }, [isInview, animate]);
  return (
...
const JobIntroSectionMotionDiv = ({
  ...
  const y = useTransform(scrollYProgress, [0.15 * idx, 0.3 + 0.15 * idx], [startValue, 0]);
  const opacity = useTransform(scrollYProgress, [0.15 * idx, 0.3 + 0.15 * idx], [0, 1]);
  return (
    <motion.div
      style={{
        y,
        ...
        <div className="relative mt-[40vh] w-full h-[100vh] flex justify-center items-center">
          ...
          >
            <div className="Circle absolute w-full aspect-square">
              <Circles ulMotion={{ css: 'animate-spin-slow' }} liMotion={{ css: 'w-[110%]' }} />
            </div>
            <span
...
export default function About() {
  return (
    <Layout seoTitle="ABOUT" nav={{ isCollapsed: true }}>
