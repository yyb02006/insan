...
export default function useInfiniteScrollFromFlatform({
  ...
  const [nextPageToken, setNextPageToken] = useState<string>(initialNextPageToken);
  ...
  const addData = async () => {
    ...
    try {
      if (category === 'filmShort' && !onSelectedList && apiPage[category] <= page && hasNextPage) {
        ...
        setSearchResults((p) => ({
          ...
          [category]: [
            ...p[category],
            ...mergedVimeoVideos.filter((el: VimeoVideos) => ciIncludes(el.name, snapshot)),
          ],
        }));
      ...
      } else if (category === 'outsource' && !onSelectedList && apiPage[category] <= page) {
        const apiKey = process.env.NEXT_PUBLIC_YOUTUBE_API_KEY;
        if (nextPageToken !== '') {
          const youtubeData: { nextPageToken: string; items: GapiItem[] } = await (
            await fetch(
          ...
          const mergedYoutubeData: ExtendGapiItems[] = youtubeData.items.map((item: GapiItem) => ({
            ...item,
...
export function useInfiniteScroll<T = unknown>({
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(handleIntersection, observerHandlerConfig);
    const currentIntersectionRef = intersectionRef.current;
    if (currentIntersectionRef) {
      observer.observe(currentIntersectionRef);
    }
